...
class ListDataloadersWrapper:
    ...
    def __next__(self):
        # choose one batch from each dataloader
        batches = []
        for i in range(self.num_dataloaders):
            try:
                batches.append(next(self.iterators[i]))
            except StopIteration:
                print(f"The dataloader {i} is empty, restarting all dataloaders")
                self.counters = [0] * self.num_dataloaders
...
losses = {task: [] for task in [0, 1, 2, 3, 'multi']} # 0: mlm, 1: nsp, 2: dc, 3: tc
...
for epoch in range(remaining_epochs):
    ...
    for batches in p_bar:
        task_indexes = [int(batch["task_index"][0].item()) for batch in batches]
        batches = [{k: v.to(device) for k, v in batch.items()} for batch in batches]
        loss_multi = 0.0
        for task_index, batch in zip(task_indexes, batches):
            loss = model(**dict(batch = batch, current_step = current_step))
            loss = loss.mean() / len(batches)
            loss_multi += loss
        ...
        optimizer.zero_grad()
        current_step += 1
        losses['multi'].append(loss_multi.item())
        ...
        if args.verbose:
            ...
            s_postfix += f"MultiTask: {np.mean(losses['multi'])}  "
        ...
        if current_step % args.logging_steps == 0:
            ...
            s_print += f"MultiTask: {np.mean(losses['multi'])}  \t"
    ...
    elif args.task_best_model_checkpoint == "multi":
        val_loss = val_multi_loss
    else:
